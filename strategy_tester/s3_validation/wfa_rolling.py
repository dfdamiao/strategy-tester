"""Rolling walk-forward validation. Strimpel Ch.6."""
...
def build_rolling_folds(
    ...
    """Fixed IS + OOS windows sliding forward."""
    n = len(common_idx)
    ...
    folds = []
    start = 0
    while start + total <= n and len(folds) < max_windows:
        is_idx = common_idx[start: start + is_days]
        oos_idx = common_idx[start + is_days: start + total]
        if len(oos_idx) < 10:
            break
...
def _process_one_pair_wfa_rolling(
    pair_row: dict,
    is_days: int,
    oos_days: int,
    max_windows: int,
    ...
    """Process a single pair through rolling WFA folds.
    ...
    folds = build_rolling_folds(common, is_days, oos_days, max_windows)
    if not folds:
        return None
    ...
    return {
        ...
        "val_method": "wfa_rolling",
        "wfe": round(mean_oos / mean_is, 4) if mean_is > 0 else 0.0,
    }


def _process_one_pair_wfa_rolling_wrapper(args: tuple) -> dict | None:
    """Unpack tuple for imap_unordered compatibility."""
    return _process_one_pair_wfa_rolling(*args)
...
def wfa_rolling(
    ...
    **config,
    ...
    """Rolling walk-forward: IS=504d, OOS=180d, up to 30 windows."""
    is_days = config.get("wfa_rolling_is", 504)
    oos_days = config.get("wfa_rolling_oos", 180)
    max_windows = config.get("wfa_max_windows", 30)
    ...
    tasks = [
        (pr, is_days, oos_days, max_windows, fees, slope_window,
    ...
    if n_pairs < 20:
        ...
        for i, args in enumerate(tasks):
            result = _process_one_pair_wfa_rolling(*args)
            if result is not None:
                rows.append(result)
            if (i + 1) % 10 == 0 or i == n_pairs - 1:
                n_pass = sum(1 for r in rows if r["passed"])
                print(
                    f"    wfa_rolling: {i + 1}/{n_pairs} "
        ...
        with ctx.Pool(
            ...
            for i, result in enumerate(pool.imap_unordered(
                _process_one_pair_wfa_rolling_wrapper, tasks, chunksize=8,
            )):
                if result is not None:
                    rows.append(result)
